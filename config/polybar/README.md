# Polybar Configuration (dwm)

This directory contains the Polybar status bar configuration bundled with dwm. It is **self-contained** — you do not need to clone any external repos.
...
When you run `sudo make install` from the dwm root, Polybar configs are copied to:
```
~/.config/polybar/
```
